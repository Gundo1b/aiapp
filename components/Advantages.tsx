        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {advantages.map((adv, index) => (
            <ScrollReveal key={adv.category} delay={index * 100}>
              <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full group relative z-10">
                <div className="flex items-center gap-4 mb-4">
                  <div className="p-3 rounded-xl bg-blue-50 dark:bg-slate-800 text-primary dark:text-blue-400 group-hover:bg-primary group-hover:text-primary-foreground transition-colors duration-300 group-hover:scale-110 group-hover:rotate-6 transform">
                    {iconMap[adv.category]}
                  </div>
                  <h3 className="font-bold text-xl text-slate-900 dark:text-slate-200">{adv.category}</h3>
                </div>
